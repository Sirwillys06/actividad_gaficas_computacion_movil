...
class _DashboardChart extends StatefulWidget { final int type; const _DashboardChart(this.type); @override State<_DashboardChart> createState() => _DashboardChartState(); }
class _DashboardChartState extends State<_DashboardChart> {
  double filter = 0; bool alternate = false; String query = ''; int page = 0; Timer? timer; int tick = 0;
  @override void dispose() { timer?.cancel(); super.dispose(); }
  @override Widget build(BuildContext context) {
    final d = SportsApiMock.getMockStandings();
    final points = d.points.where((p) => p.value >= filter && (query.isEmpty || p.label.toLowerCase().contains(query.toLowerCase()))).toList();
    final chart = widget.type == 24 ? ScatterChart(ScatterChartData(scatterSpots: points.map((p) => ScatterSpot(p.value, p.secondaryValue ?? 0, radius: 6)).toList()))
      : widget.type == 23 || widget.type == 28 || widget.type == 30 ? PieChart(PieChartData(centerSpaceRadius: widget.type == 30 ? 35 : 0, sections: points.map((p) => PieChartSectionData(value: p.value, title: p.label, radius: 60)).toList()))
      : BarChart(BarChartData(barGroups: points.skip(page * 5).take(5).toList().asMap().entries.map((e) => BarChartGroupData(x: e.key, barRods: [BarChartRodData(toY: alternate ? e.value.secondaryValue ?? 0 : e.value.value, color: Colors.teal, width: 16)])).toList()));
    return ChartCardWrapper(title: 'Dashboard avanzada ${widget.type - 20}', subtitle: 'Datos TheSportDB / fallback offline', chart: Column(children: [Expanded(child: chart), if (widget.type == 24) Slider(value: filter.clamp(0, 100), max: 100, onChanged: (v) => setState(() => filter = v)), if (widget.type == 32) TextField(decoration: const InputDecoration(labelText: 'Buscar equipo'), onChanged: (v) => setState(() => query = v)), if (widget.type == 25 || widget.type == 26) SwitchListTile(title: const Text('Modo alternativo'), value: alternate, onChanged: (v) => setState(() => alternate = v)), if (widget.type == 27) Row(mainAxisAlignment: MainAxisAlignment.center, children: [IconButton(onPressed: page > 0 ? () => setState(() => page--) : null, icon: const Icon(Icons.chevron_left)), Text('Página ${page + 1}'), IconButton(onPressed: () => setState(() => page++), icon: const Icon(Icons.chevron_right))])])));
  }
}
